...
class FloweringPlant(Plant):
    """Represents flowering plant that extends Plant with
    color and bloom status"""
    ...
    def base_info(self) -> str:
        """Return flowering plant information including
        flower color and bloom status."""
        if self.is_blooming:
            return (f"{self.name}: {self.height}cm, "
...
class GardenManager:
    ...
    def create_garden_network(cls):
        """Create a pre-configured garden network
        with Alice and Bob's gardens"""
        maneger = cls()
    ...
    def calculate_score(self, garden_name: str):
        """Calculate total score for a garden based on plant heights e points.
...
if __name__ == "__main__":
    ...
    print(
        f"Garden scores - Alice: {manager.calculate_score('Alice')}, "
        + f"Bob: {manager.calculate_score('Bob')}")
